...
const CreatePO = () => {
  ...
  const generateDraftNumber = () => {
    // Draft POs get a temporary number; the final number is assigned on finalization
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Autorisation d'engagement : key user/admin libres, participant → son
    // plafond, créateur du budget → la limite de son rôle, autres → refus.
    ...
    try {
      if (isEditMode && editId) {
        ...
        // Create new PO
        const po_number = generateDraftNumber();
