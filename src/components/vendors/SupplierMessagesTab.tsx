...
const db = supabase as any; // supplier_messages pas encore dans les types générés

const fmtTime = (s: string) =>
  new Date(s).toLocaleString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' });

/** Onglet Messages côté équipe (fiche fournisseur) : lit/répond au fil du fournisseur. */
const SupplierMessagesTab = ({ supplierId }: { supplierId: string }) => {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!user || !organizationId) throw new Error('Session invalide');
      const { data: profile } = await supabase.from('profiles').select('full_name').eq('id', user.id).maybeSingle();
      const senderName = profile?.full_name || user.email || 'Équipe';

      const { error } = await db.from('supplier_messages').insert({
      ...
      toast({ variant: 'destructive', title: 'Envoi impossible', description: e?.message });
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-[60vh] rounded-lg border border-border bg-card">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
        ...
        <span className="font-medium text-sm">Conversation avec le fournisseur</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading ? (
          ...
          messages.map((m) => {
            const mine = m.sender_type === 'internal'; // côté équipe : nos messages à droite
                ...
                <div className={cn('max-w-[80%] rounded-2xl px-3.5 py-2 text-sm',
                  ...
                  {!mine && (
                    <div className="flex items-center gap-1 text-[11px] font-medium mb-0.5 opacity-80">
                      ...
                      {m.sender_name || (isBot ? 'Assistant' : 'Fournisseur')}
                    </div>
                  )}
      ...
      <div className="border-t border-border p-3">
        <div className="flex items-end gap-2">
          <Textarea
            ...
            placeholder="Répondre au fournisseur…"
