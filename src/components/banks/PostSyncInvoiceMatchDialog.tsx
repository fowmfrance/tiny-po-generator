...
import type { InvoiceLinkProposal } from '@/utils/invoiceMatch';

/**
 * Revue des rapprochements facture ↔ opération proposés après la synchro
 * bancaire (même ergonomie que la revue des tiers) : uniquement des montants
 * exacts, tout est coché par défaut, l'utilisateur décoche puis valide en masse.
 * Le code projet remonte du BdC de la facture au moment du rapprochement.
 */
export default function PostSyncInvoiceMatchDialog({
  proposals,
  isApplying,
  onConfirm,
  onClose,
}: {
  proposals: InvoiceLinkProposal[];
  isApplying: boolean;
  onConfirm: (accepted: InvoiceLinkProposal[]) => void;
  onClose: () => void;
}) {
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {proposals.length} facture{proposals.length > 1 ? 's' : ''} au montant exact
          </DialogTitle>
          <DialogDescription>
            Ces opérations correspondent au centime près à une facture reçue de leur
            fournisseur. Rapprocher fige le code projet et marque la facture payée.
          </DialogDescription>
        </DialogHeader>
            ...
            {proposals.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 max-w-[46%] min-w-0">
                    <span className="text-muted-foreground text-xs">→</span>
                    <FileText className="h-3.5 w-3.5 text-brand shrink-0" />
                    <span className="text-sm font-medium text-brand truncate">
                      {p.invoice.invoiceNumber || 'Sans numéro'}
                    </span>
                    {p.invoice.projectCode && (
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button onClick={() => onConfirm(accepted)} disabled={isApplying || accepted.length === 0}>
            ...
            Rapprocher {accepted.length > 0 ? `les ${accepted.length}` : ''}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
